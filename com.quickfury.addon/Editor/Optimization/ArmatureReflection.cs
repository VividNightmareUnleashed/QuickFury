...
namespace QuickFury {
    ...
    internal static class ArmatureReflection {
        ...
        internal static void Resolve() {
            ...
            ArmatureLinkApply = VrcfuryCompatibility.FindNoArgVoid(armatureType, "Apply");
            ...
            HapticSocketsApply = VrcfuryCompatibility.FindNoArgVoid(hapticType, "Apply");
            HapticSocketsAvatarField = hapticType?.GetField(
                "avatarObject",
                BindingFlags.Instance | BindingFlags.NonPublic
            );

            VfGameObjectType = VrcfuryCompatibility.FindType("VF.Utils.VFGameObject");
            gameObjectField = VfGameObjectType?.GetField(
                "_gameObject",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            GetConstraintsMethod = VrcfuryCompatibility.FindUniqueMethod(
                VfGameObjectType,
                "GetConstraints",
                method => {
            ...
            RemoveFromPhysbones = VrcfuryCompatibility.FindUniqueMethod(
                physboneUtilsType,
                "RemoveFromPhysbones",
                method => {
                    var parameters = method.GetParameters();
